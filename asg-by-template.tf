resource "aws_launch_template" "ecs-codedang-template" {
  # Name of the launch template
  name = "codedang-template"
  ...
  tag_specifications {
    ...
    tags = {
      Name = "launch template for codedang-ecs"
    }
  }
}
...
resource "aws_autoscaling_group" "codedang-asg" {
  ...
  vpc_zone_identifier   = [aws_subnet.test-pub_subnet.id]
  ...
  mixed_instances_policy {
    ...
    launch_template {
      launch_template_specification {
        launch_template_id = aws_launch_template.ecs-codedang-template.id
        version            = "$Latest"
      }
      override {
        instance_type     = "t4g.small"
